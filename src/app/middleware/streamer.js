...
class Streamer extends events.EventEmitter {

    constructor(){
        super();

        this.on("badFile", (res, e) => {
        ...
        this.on("badRange", (res, e) => {
            this.errorHeader(res, 416);
            res.end("<!DOCTYPE html><html lang=\"en\">" +
        ...
        this.on("security", (res, e) => {
            this.errorHeader(res, 403);
            res.end("<!DOCTYPE html><html lang=\"en\">" +
        ...
        this.on("badRequest", (res, e) => {
            this.errorHeader(res, 400);
            res.end("<!DOCTYPE html><html lang=\"en\">" +
    ...
    call (req, res) {
        ...
        if (!info.path) {
            this.emit("badFile", res);
            return false;
        } else if (info.path.search(/^\.\.?|^\/|^\\/) !== -1) {
            this.emit("security", res, { message: info.path });
            return false;
            ...
            if (ext === null || ext.length !== 2 || (info.mime = mimeTypes[ext[1].toLowerCase()]) === undefined) {
                this.emit("badMime", res, { message: info.path });
                return false;
            }
        }
        ...
        if (settings.random) {
            if (!this.randomFile(info)) {
                this.emit("noRandomFiles", res, { message: info.path });
                return false;
            }
        } else {
        ...
        if (range !== undefined && (range = range.match(/bytes=(.+)-(.+)?/)) !== null) {
            ...
            info.start = this.isNumber(range[1]) && range[1] >= 0 && range[1] < info.end ? range[1] - 0 : info.start;
            info.end = this.isNumber(range[2]) && range[2] > info.start && range[2] <= info.end ? range[2] - 0 : info.end;
            ...
            info.start = this.isNumber(reqUrl.query.start) && reqUrl.query.start >= 0 && reqUrl.query.start < info.end ? reqUrl.query.start - 0 : info.start;
            info.end = this.isNumber(reqUrl.query.end) && reqUrl.query.end > info.start && reqUrl.query.end <= info.end ? reqUrl.query.end - 0 : info.end;
        }
        ...
        if (info.start > 0 && info.mime === "video/x-flv") {
            res.write("FLV" + this.pack("CCNN", 1, 5, 9, 9));
        }
    ...
    randomFile (info) {
        ...
        randomFiles = randomFiles.filter( (el) => { return el.substr(-extn.length) === extn; });
